output "lab" {
  value = {
    ...
    target               = { host = aws_rds_cluster.db["target"].endpoint, port = 5432, master_secret_arn = aws_rds_cluster.db["target"].master_user_secret[0].secret_arn, dms_secret_arn = aws_secretsmanager_secret.dms["target"].arn }
  }
}
